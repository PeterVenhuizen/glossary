		...
		<main>
            ...
            <?php
                if (isset($_GET['id'])) {
                    ...
                    if ($stmt->rowCount() > 0) {
                        ...
                        echo '  <form action="" method="POST" id="form_post">
                                    
                                    <label for="form_topic" class="form_label">Topic:</label>
                                    <input type="text" name="form_topic" id="form_topic" value="' . $row['topic'] . '">
                                    
                                    <textarea class="ckeditor" name="form_body" id="form_body">' . html_entity_decode($row['body']) . '</textarea>
                                    
                                    <label for="form_keywords" class="form_label">Keywords:</label>
                                    <input type="text" name="form_keywords" id="form_keywords" value="' . $row['tags'] . '">
            ...
            <?php
                if (isset($_POST['form_submit'])) {
                    ...
                    $query_params = array(':topic' => $topic, ':keywords' => $keywords, ':body' => $body, ':id' => $_GET['id']);
                    try {
                        $stmt = $db->prepare($query);
                        $stmt->execute($query_params);
